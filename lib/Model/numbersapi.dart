...
const _headers = {
  ...
  'x-rapidapi-key': 'a9a982829bmsh8b99666fb3a72bbp1cbbb3jsn747e577ba242'
};
...
Future<String> newFactForDate(DateTime date) async {
  final response = await http.get(
      Uri.parse(
          'https://numbersapi.p.rapidapi.com/${date.month}/${date.day}/date'),
      headers: _headers);
...
Future<String> newFactForYear(int year) async {
  final response = await http.get(
      Uri.parse('https://numbersapi.p.rapidapi.com/$year/year'),
      headers: _headers);
  if (response.statusCode == 200) {
    // If server returns an OK response, parse the JSON.
    return response.body;
